...
int main(void) {
    ...
    fd[0] = open(PIPENAME, O_RDONLY | O_NONBLOCK); // send 전용
    fd[1] = open(PIPENAME, O_WRONLY | O_NONBLOCK); // recv 전용
    fd[2] = open(PIPENAME, O_RDWR | O_NONBLOCK); // result send 전용
    ...
    // if (write(fd[0], (void*)&sndbuf, sizeof(struct sendbuf)) == -1) {
    //    perror("msgsnd error : ");
    //    return 0;
    //}
    //sleep(2);
    //if (read(fd[1], (void*)&sndbuf, sizeof(struct sendbuf)) == -1) {
    //   perror("msgrcv error : ");
    //    return 0;
    //}
...
void send_result(int clnt_pid){
	...
	//if(write(p[2], (void*)&rsbuf, sizeof(struct resultbuf)) < 0)
	//{
