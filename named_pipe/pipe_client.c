...
int fd[3]; // 파일 디스크립터 3개 생성 // 1. 읽기, 2. 쓰기, 3. 모르겠음... 일단 2개 만 사용
char msg[MSG_SIZE];
...
int main(void) {
    ...
    /* named pipe not exist -> make named pipe */
    if(mkfifo(PIPEPATH1, S_IRWXU|S_IRWXG|S_IRWXO) == -1) {
        printf("Pipe파일이 존재하지 않습니다.\nPipe 파일을 생성합니다.");
        mkfifo(PIPEPATH1, S_IRWXU|S_IRWXG|S_IRWXO); // send 
        mkfifo(PIPEPATH2, S_IRWXU|S_IRWXG|S_IRWXO); // recv
        mkfifo(PIPEPATH3, S_IRWXU|S_IRWXG|S_IRWXO); // result
    }
    /* named pipe 열기, Write 전용으로 열기 */
    key_id1 = open(PIPENAME, O_RDONLY | O_NONBLOCK); // send 전용
    key_id2 = open(PIPENAME, O_WRONLY | O_NONBLOCK); // recv 전용
    key_id3 = open(PIPENAME, O_RDWR | O_NONBLOCK); // send, recv 전용
    
    if (key_id1 < 0 || key_id2 < 0 || key_id3 < 0) {
...
void* read_data()
{
	...
	if (read(key_id1, (void*)&rcvbuf, MSG_SIZE+1) < 0)
	{
		perror("msgrcv error : ");
		return 0;
	}
...
void* write_input(void* data)
{
	...
	if (write(key_id2, (void*)&sndbuf, sizeof(struct sendbuf)) == -1)
	{
		perror("msgsnd error : ");
		return 0;
	}
